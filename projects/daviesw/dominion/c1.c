...
int testSmithy () {
    ...
    if ((testState.hand[thisPlayer][handPos] == state.hand[thisPlayer][ (state.handCount[thisPlayer] - 1)])) {
    ...
    if ((testState.hand[thisPlayer][ (testState.handCount[thisPlayer] - 3)] == expectedCards[0])
    && (testState.hand[thisPlayer][ (testState.handCount[thisPlayer] - 2)] == expectedCards[1])
    && (testState.hand[thisPlayer][ (testState.handCount[thisPlayer] - 1)] == expectedCards[2])) {
        printf("CARDS DRAWN ARE FORMER TOP THREE FROM PLAYERS DECK TEST: PASSED\n");

    }
    else {
        printf("CARDS DRAWN ARE FORMER TOP THREE FROM PLAYERS DECK TEST: FAILED\n");
        testPassed = 1;
    }
