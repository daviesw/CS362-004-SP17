...
int testVillage (int thisPlayer, struct gameState testState, int handPos) {
    ...
    int expectedCards[1] = {testState.deck[thisPlayer][(deckCounter - 1)]};
    //printf("About to memcpy\n"); 
    memcpy(&state, &testState, sizeof(struct gameState));
    
    printf("\n\n--------------TESTING VILLAGE----------------------\n\n");
    
    //villagePlayed(&testState, thisPlayer, handPos);
    village_fct(thisPlayer, &testState, handPos);
    //cardEffect(village, 0, 0, 0, &testState, handPos, &bonus);
    //printf("Made it here\n");
    ...
    if ((testState.hand[thisPlayer][handPos] == expectedCards[0])) {
        ...
        printf("EXPECTED CARD: %d ACTUAL CARD: %d\n", expectedCards[0], testState.hand[thisPlayer][handPos]);
	testPassed = 0;
    }
    ...
    if ((testState.hand[thisPlayer][handPos] == expectedCards[0])) {
        //printf("CARD DRAWN IS FORMER TOP OF PLAYER'S DECK TEST AND IN CORRECT POSITION TEST: PASSED\n");

    }
    else {
        printf("CARD DRAWN IS FORMER TOP OF PLAYER'S DECK AND IN CORRECT POSITION TEST: FAILED\n");
        printf("EXPECTED: %d ACTUAL: %d\n", expectedCards[0], testState.hand[thisPlayer][handPos]);
	testPassed = 1;
    }
    if (testPassed == 1) {
...
//    	    printf("VICTORY AND KINGDOM CARDS NOT AFFECTED TEST: FAILED\n");
    ...
    if(testState.numActions == (state.numActions + 2)) {
        printf("NUMBER OF ACTIONS INCREASED BY TWO TEST: PASSED\n");
    }
    else {
        printf("NUMBER OF ACTIONS INCREASED BY TWO TEST: FAILED\n");
...
int main() 
{
	...
	//printf("numPlayers: %d player: %d\n\n", G.numPlayers, p);
        result = testVillage(p, G, handPos);
